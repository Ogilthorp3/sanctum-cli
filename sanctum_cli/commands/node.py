...
@node_app.command(
    "bootstrap-script",
    help="Generate the one-run bootstrap script for the new Mac's keyboard.",
)
def node_bootstrap_script(
    ...
    out: Annotated[
        ...
        typer.Option(
            "--out",
            help="Write the script here (0755) instead of stdout.",
            dir_okay=False,
        ),
    ] = None,
) -> None:
    """Emit the bootstrap script, minting the automation key pair on first run.

    Default is stdout (script bytes only — pipe/redirect clean); the transfer
    guidance goes to stderr. ``--out`` writes an executable file instead. The
    embedded public key is ``~/.ssh/sanctum_automation.pub``, generated via
    ssh-keygen if this console has never bootstrapped a satellite before.
    """
    try:
    ...
    if out is not None:
        ...
        console.print(
            "  [dim]get it to the new Mac (AirDrop / USB), run it there in Terminal, "
    ...
    print(script, end="")
    err_console.print(
        "[dim]save this, get it to the new Mac (AirDrop / USB), run it there in "
        "Terminal, then `sanctum node adopt <its-ip>` from here.[/]"
    )
