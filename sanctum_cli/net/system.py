...
def firewalla_box_preflight(gateway: str, key: str, user: str = "pi") -> tuple[bool, bool]:
    ...
    (:func:`_ssh_argv`) the cutover's box ops use, so it checks the exact transport the
    ``sudo dhclient`` re-lease will run on. The remote prints a distinct marker for
...
#
# The mutating tags the single-NAT DMZ orchestrator fires (mirrors
# sanctum_cli.devices.intents._RUNNER_WAN_DHCP / _LEASE_OBSERVE / _ARMOR_ARM /
# _DHCP_RELEASE + the raw-readback tags). Each maps to a real firerouter command
# run on the Firewalla over the fw key. The command strings are authored from the
# kit's own watchdog (sanctum-singlenat-armor/bin/singlenat-watchdog.sh +
# singlenat-verify.sh) so the CLI's runner and the armor's self-heal issue the
# SAME firerouter ops:
#
#   * wan_dhcp     — switch the WAN to DHCP/PPPoE passthrough: derive the WAN dev
#                    from the default route, release + re-acquire a DHCP lease on it.
#   * lease_observe— READ the downstream WAN's primary IPv4 and RETURN it so
#                    flip.classify_wan_ip can classify the lease. The ONE read.
...
def _fw_mutate_via_ssh(gateway: str, key: str, tag: tuple[str, ...]) -> str:
    ...
    in stdout) so the flip can classify the lease; for a raw-readback tag
    (``wan_addr_cidr`` / ``wan_routes``) returns the stdout verbatim (prefix +
    routes preserved for the poison gate); for the others returns "" on success
    (the orchestrator inspects only that they did not raise).
    """
    remote = _FW_MUTATING_REMOTE[tag]
    argv = _ssh_argv(gateway, key, "pi", remote)
    try:
        proc = subprocess.run(
...
def make_real_runner(*, fw_gateway: str | None, fw_key: str | None) -> Runner:
    ...
    def runner(tag: tuple[str, ...]) -> str:
        # The single-NAT mutating + raw-readback ops are the APPLY-PATH tags. They
        # are NOT cached (each must re-execute — a re-lease/observe is a fresh box
        # action every time, unlike the read-only WAN/ports probes above) and they
        # fail-CLOSED: no fw gateway+key resolved RAISES rather than a silent "" that
        # would report a green cutover the box never received.
