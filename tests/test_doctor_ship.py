...
def test_evaluate_green_when_all_pass() -> None:
    res = evaluate(
        ...
        registry=_reg(),
        adapters={
            "keychain_has": lambda a, s: True,
            "is_default": lambda a, s: False,
            "heal_action_ok": lambda l: True,
            "sink_live": lambda n: True,
            "probe_is_false_green": lambda p: False,
            "soak_days": lambda m: 8.0,
            "soak_clean": lambda m: True,
            "docs_resolves": lambda u: True,
            "demo_exits_zero": lambda c: True,
        },
    )
...
def test_gate_soak_green_from_written_clean_result(
    ...
    assert result.status is GateStatus.GREEN, (
        f"Expected GREEN, got {result.status}: {result.detail}"
    )
